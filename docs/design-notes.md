...
> 交付阶段：Day 2-3 设计说明文档
> 目标：回答面试题要求的三个核心问题：渲染方案、状态管理核心数据结构、多人协作 CRDT 扩展方案。
...
> 我选择 React Flow + SVG + DOM 的混合方案。这个项目需要的是“画布式工作区”，不是一定要用 HTML `<canvas>`。节点使用 DOM / React 组件渲染，因为节点里可能有标题、描述、状态徽章、代码块等复杂内容；节点之间的连线使用 SVG，因为 SVG 更适合画路径、箭头和动画；拖拽、缩放、平移、选择和 MiniMap 交给 React Flow。HTML `<canvas>` 性能好，但复杂交互和富内容维护成本太高；纯 DOM 不适合画复杂连线；纯 SVG 不适合复杂节点，所以我选择混合方案。

> 状态管理上，我会用标准化的 blocks map，而不是简单数组。核心结构包括 workspace.blocks、blockOrder、selectedBlockIds、manualLayoutMap、agent.status、messages、patchLog、pendingTransaction、undoStack 和 redoStack。Agent 不直接改 UI，而是输出 AgentPatch，前端负责校验 Patch、解决冲突、应用增量更新，并把每次操作记录成 transaction，方便撤销、重做和失败回滚。

> 如果要支持多人协作，我会把本地 Zustand 状态升级为 Yjs CRDT 状态。Y.Doc 作为唯一真实数据源，blocks 使用 Y.Map，blockOrder 使用 Y.Array，用户光标和选区用 Awareness。用户操作和 Agent Patch 都作为 CRDT transaction 写入 Y.Doc。冲突处理从简单覆盖升级为字段级合并，例如 Agent 可以更新节点标题，但不能默认覆盖用户手动拖拽后的位置。Undo / Redo 也要区分本地撤销和全局协作历史。
...
Zustand 负责本地状态管理
AgentPatch 负责增量更新
Transaction 负责撤销、重做和回滚
Yjs 负责未来多人协作
```

这套架构能解释清楚真实工程里的关键问题：

- 为什么需要画布式工作区，但不把 HTML `<canvas>` 作为主渲染层？
- Agent 如何安全地更新 UI？
- 用户手动调整为什么不会被覆盖？
- 状态如何支持撤销和回滚？
- 多人协作时如何避免冲突？

最终目标不是做一个只能演示一次的玩具，而是做一个可以继续扩展成真实 AI Workspace 产品的稳定架构。
